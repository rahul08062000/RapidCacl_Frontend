...
const GridRow = ({ rowIndex, cellSize, shiftAnimation, totalRow }) => {
  const { colHeader, inputValues, focusRowIndex, focusColIndex, correctCells, incorrectCells, setFocusRowIndex, setFocusColIndex } = useContext(AppContext);
  ...
  return (
    <Animated.View style={[styles.row]}>
      {colHeader.map((_, colIndex) => {
        ...
        return (
          <View key={colIndex} style={{ zIndex: isFocused ? 3 : 0 }}>
            <Cell
              rowIndex={rowIndex}
              colIndex={colIndex}
              value={inputValues[cellKey] || ''}
